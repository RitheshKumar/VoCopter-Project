        ...
        for (int i = 0; i < numOutputChannels; ++i) {
            ...
            if (inputChannelData[i] != nullptr) {
                ...
                for (int j=0; j < numSamples; j++) {
                    x1.at(j) = 10.0f*inputChannelData[i][j];
                    myTestVec.at(j) = 10.0f*inputChannelData[i][j];
                    //std::cout<<inputChannelData[i][j]<<" "<<myTestVec.at(j)<<std::endl;
                }
                ...
                /*std::copy( &inputChannelData[numInputChannels-1][0], &inputChannelData[numInputChannels-1][0]+numSamples, x2.begin()+numSamples );
                std::copy(x2.end()-numSamples, x2.end(), x1.begin());
                std::copy( &inputChannelData[numInputChannels-1][0], &inputChannelData[numInputChannels-1][0]+numSamples, myTestVec.begin()+numSamples );
